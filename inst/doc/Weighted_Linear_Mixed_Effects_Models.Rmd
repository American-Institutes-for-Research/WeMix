...
While any likelihood that can be written down can be maximized, efficent calculation is always appreciated by analyists. In particular, the weighted hierarchical linear model has nested integrals that are difficult to compute efficently. For unweighted data, Bates and Pinheiro (1998) show a method of calculation that leaves only a probability density function in the integral, which can be easily integrated as the multiplicative identity, obviating the need for numerical integration.

This paper generalizes the method in Bates and Pinheiro (1998) and the `lme4` package (Bates, Maechler, Bolker, and Walker, 2015) to the weighted case. I am unaware of a description of this and will detail the information available in existing statistical software documentation which tends to simply cite Rabe-Heskety and not provide additional detail on how weighted estimates are formed.
...
\end{align}
where $\bm{Z}^{(l)} \in \mathbb{R}^{n \times k_z^{(l)}}$ are block matrixes that form $\bm{Z}$

\begin{align}
\bm{Z} = \left[ \bm{Z}^{(2)}, ..., \bm{Z}^{(L)} \right]
\end{align}
...
Instead the weights are often adjusted to capture both the unequal selection probabilities and total sample size. These  methods are documented in Pfeffermann, Skinner, Holmes, Goldstein, & Rasbash (1998) and Rabe-Hesketh & Skrondal (2006), and other places.
...
I would commend the authors of the Stata manuals for routinely producing extensive documentation on methods and formulas of their software. Uncharacteristicly, in the case of weighted HLM they simply state, "Weighted estimation is achieved through incorporating $w_j$ and $w_{i|j}$ into the matrix decomposition methods detailed above to reflect replicated clusters for $w_j$ and replicated observations within clusters for $w_{i|j}$. (Stata, page 534)"\footnote{See "Methods and Formulas" for "mixed---Multilevel mixed-effects linear regression" in "[ME] Stata Multilevel mixed-effects reference manual: release 17."}
...
\begin{align} 
r^2(\bm{\theta}, \bm{\beta}, \bm{\upsilon}) &= \lvert\lvert \bm{y}-\bm{X\beta} - \bm{Z\Lambda(\theta)\upsilon}\rvert\rvert^2 + ||\bm{\upsilon}||^2 \label{eq:pseudoSS}
\end{align} 
...
\end{align} 
Plugging eq. \ref{eq:uwA} into eq. \ref{eq:WeMixR0} and finding the least squares solution to
\begin{align} 
\bm{A} \left[ \begin{matrix} \bm{\upsilon} \\ \bm{\beta} \end{matrix} \right]&=\left[ \begin{matrix} \bm{y} \\ \bm{0} \end{matrix} \right] \label{eq:lsqnolsq}
\end{align}
...
\begin{align}
\bm{QR} \left[ \begin{matrix} \bm{\upsilon} \\ \bm{\beta} \end{matrix} \right]&=\left[ \begin{matrix} \bm{y} \\ \bm{0} \end{matrix} \right]
\end{align} 
...
\end{align}
Where $\bm{\hat{u}}$ and $\bm{\hat{\beta}}$ are the least squares solution to eq. \ref{eq:lsqnolsq}. Using the identity that for any vector $\bm{v}$ the sum of squares is also just the inner product, so $||\bm{v}||^2 = \bm{v}^T\bm{v}$,  
\begin{align}
...
\begin{align}
(\bm{y},\bm{\upsilon}) &\sim N(\bm{y} - \bm{X\beta} - \bm{Z\upsilon},\sigma^2 \bm{I}_{n}) * N(\bm{\upsilon},  \sigma^2 \bm{I}_{k_z})
\end{align}
and the probability density function of the joint distribution of $\bm{y}$ and $\bm{\upsilon}$ is
\begin{align}
f_{\bm{y}, \bm{\upsilon}} \left(\bm{y}, \bm{\upsilon}, \bm{\beta}, \bm{\theta}, \sigma^2 \right)&= \frac{1}{\left(2\pi\sigma^2\right)^{\frac{n}{2}}} \exp{\left[\frac{- (\bm{y} - \bm{X\beta} - \bm{Z\upsilon})^2 }{2\sigma^2} \right]}  \cdot \frac{1}{\left(2\pi\sigma^2\right)^{\frac{k_z}{2}}} \exp{\left[ \frac{-||\bm{\upsilon}||^2}{2\sigma^2}\right] } 
\end{align}
pluggin in eq. \ref{eq:pseudoSS}
\begin{align}
f_{\bm{y}, \bm{\upsilon}} \left(\bm{y}, \bm{\upsilon}, \bm{\beta}, \bm{\theta}, \sigma^2 \right)&=&= \frac{1}{\left(2\pi\sigma^2\right)^{\frac{n}{2}}} \exp{\left[\frac{-r^2(\bm{\theta}, \bm{\beta}, \bm{\upsilon}) + ||\upsilon||^2}{2\sigma^2} \right]}  \cdot \frac{1}{\left(2\pi\sigma^2\right)^{\frac{k_z}{2}}} \exp{\left[ \frac{-||\bm{\upsilon}||^2}{2\sigma^2}\right] }  \\
&= \frac{1}{\left(2\pi\sigma^2\right)^{\frac{n+k_z}{2}}} \exp{\left[\frac{-r^2(\bm{\theta}, \bm{\beta}, \bm{\upsilon}) }{2\sigma^2}\right] } 
\end{align}
...
&=\int \frac{1}{\left(2\pi\sigma^2\right)^{\frac{n+k_z}{2}}} \exp{\frac{-r^2(\bm{\theta}, \bm{\beta}, \bm{\upsilon}) }{2\sigma^2} }  d\bm{\upsilon}\\
&=\frac{1}{\left(2\pi\sigma^2\right)^{\frac{n+k_z}{2}}} \exp{\frac{-r^2(\bm{\theta}, \bm{\hat{\beta}}, \bm{\hat{\upsilon}}) - \left|\left|  \bm{R}_{22}  (\bm{\beta} - \hat{\bm{\beta}})  \right| \right|^2 }{2\sigma^2} } \int \exp{\frac{- \left|\left|  \bm{R}_{11} (\bm{\upsilon} - \hat{\bm{\upsilon}}) + \bm{R}_{12}  (\bm{\beta} - \hat{\bm{\beta}})  \right| \right|^2 }{2\sigma^2} }  d\bm{\upsilon} \label{eq:lnlInt}
\end{align}
...
\begin{align}
\mathcal{L}\left( \bm{\beta}, \bm{\theta}, \sigma^2; \bm{y} \right)&=\frac{1}{\left(2\pi\sigma^2\right)^{\frac{n+k_z}{2}}} \exp{\frac{-r^2(\bm{\theta}, \bm{\hat{\beta}}, \bm{\hat{\upsilon}}) - \left|\left|  \bm{R}_{22}  (\bm{\beta} - \hat{\bm{\beta}})  \right| \right|^2 }{2\sigma^2} } \int \exp{\frac{- \left|\left|  \gamma  \right| \right|^2 }{2\sigma^2} } | {\rm det} (\bm{R}_{11}) |^{-1} d\bm{\gamma} \\
&=\frac{1}{|{\rm det}(\bm{R}_{11})| \left(2\pi\sigma^2\right)^{ \frac{n}{2}}} \exp{\frac{-r^2(\bm{\theta}, \bm{\hat{\beta}}, \bm{\hat{\upsilon}}) - \left|\left|  \bm{R}_{22}  (\bm{\beta} - \hat{\bm{\beta}})  \right| \right|^2 }{2\sigma^2} }  \left\{ \frac{1}{\left(2\pi\sigma^2\right)^{ \frac{k_z}{2}} } \int \exp{\frac{- \left|\left|  \gamma  \right| \right|^2 }{2\sigma^2} }  d\bm{\gamma} \right\}
\end{align}
and because the term in curly braces is now of a probability density function, it integrates to unity
\begin{align}
\mathcal{L} \left( \bm{\beta}, \bm{\theta}, \sigma^2; \bm{y} \right)&=\frac{1}{|{\rm det}(\bm{R}_{11})| \left(2\pi\sigma^2\right)^{\frac{n}{2}}} \exp{\frac{-r^2(\bm{\theta}, \bm{\hat{\beta}}, \bm{\hat{\upsilon}}) - \left|\left|  \bm{R}_{22}  (\bm{\beta} - \hat{\bm{\beta}})  \right| \right|^2 }{2\sigma^2} }  \label{eq:WeMixL0}
\end{align}
...
\begin{align}
D \left( \bm{\beta}, \bm{\theta}, \sigma^2; \bm{y} \right)&= 2\,{\rm ln} {|{\rm det}(\bm{R}_{11})|} + n {\rm ln} \left(2\pi\sigma^2\right) + \frac{r^2(\bm{\theta}, \bm{\hat{\beta}}, \bm{\hat{\upsilon}}) + \left|\left|  \bm{R}_{22}  (\bm{\beta} - \hat{\bm{\beta}})  \right| \right|^2 }{\sigma^2}  \label{eq:WeMixD0} 
\end{align}
...
\begin{align}
D \left( \bm{\theta}, \sigma^2; \bm{y} \right)&= 2\,{\rm ln} {|{\rm det}(\bm{R}_{11})|} + n {\rm ln} \left(2\pi\sigma^2\right) + \frac{r^2(\bm{\theta}, \bm{\hat{\beta}}, \bm{\hat{\upsilon}}) }{\sigma^2}  
\end{align}
...
\begin{align}
\widehat{\sigma^2}&= \frac{r^2(\bm{\theta}, \bm{\hat{\beta}}, \bm{\hat{\upsilon}})}{n}  
\end{align}
giving a profile deviance that is a function only of the parameter $\bm{\theta}$:
\begin{align}
D \left( \bm{\theta}; \bm{y} \right)&= 2\,{\rm ln} {|{\rm det}(\bm{R}_{11})|} + n \left({\rm ln} \left(2\pi \widehat{\sigma^2}\right) + 1 \right)
\end{align}
the profiled deviance can then be numerically maximixed over the parameters in $\bm{\theta}$ and the plug in estimators can then be used for $\hat{\sigma^2}$ and $\bm{\beta}$.
...
\begin{align}
f_{\bm{y}|\bm{\upsilon}=\bm{\upsilon}}(\bm{y}, \bm{\upsilon}) &= \prod_{i=1}^{n} \left[ {\frac{1}{\left(2\pi\sigma^2\right)^{\frac{1}{2}}} \exp\left( -\frac{\left|\left|\bm{y}_i-\bm{X}_i\bm{\beta}-\bm{Z}_i\bm{\Lambda}(\bm{\theta})\bm{\upsilon} \right|\right|^2}{2\sigma^2}\right) } \right]^{\bm{\Omega}_{ii}} \\
 &= \prod_{i=1}^{n}  {\frac{1}{\left(2\pi\sigma^2\right)^{\frac{\bm{\Omega}_{ii}}{2}}} \exp \left(- \frac{ \left|\left| \bm{\Omega}_{ii}^{\frac{1}{2}} \left(\bm{y}_i-\bm{X}_i\bm{\beta}-\bm{Z}_i\bm{\Lambda}(\bm{\theta})\bm{\upsilon} \right) \right|\right|^2}{2\sigma^2}\right) } \\
...
\begin{align}
f_{\bm{\upsilon}}(\bm{\upsilon}) &=\prod_{j=1}^{k_z} \left[ \frac{1}{\left(2\pi\sigma^2\right)^{\frac{1}{2}}} \exp \left[- \frac{ \left|\left|  \bm{\upsilon}_j \right|\right|^2}{2\sigma^2}\right]  \right]^{\bm{\Psi}_{jj}} \\
&=\prod_{j=1}^{k_z} \frac{1}{\left(2\pi\sigma^2\right)^{\frac{1}{2}||\bm{\Psi}_{jj}||_F}} \exp \left[- \frac{ \left|\left| \bm{\Psi}_{jj}^{\frac{1}{2}}  \bm{\upsilon}_j \right|\right|^2}{2\sigma^2}\right] \\
&= \frac{1}{\left(2\pi\sigma^2\right)^{\frac{1}{2}||\bm{\Psi}_{jj}}||_F} \exp \left[- \frac{ \left|\left|  \bm{\Psi}^{\frac{1}{2}}  \bm{\upsilon} \right|\right|^2}{2\sigma^2}\right] \label{eq:WeMixWfu}
\end{align}
...
\end{align}
Notice that while the unweighted integral has $k_z$ dimensions, this weighted integral has $\sum_j \bm{\Psi}_{jj}$ dimensions---the number of (population) individuals values to integrate out.
\begin{align}
